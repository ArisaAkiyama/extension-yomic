...
(function () {
  ...
  var root = typeof globalThis !== 'undefined' ? globalThis : (WINDOW ? window : {});
...
var source = {
    name: "WestManga",
    baseUrl: "https://westmanga.co",
    apiUrl: "https://data.mantweh.online",
    language: "id",
    version: "1.0.0",
    description: "West Manga extension implemented in JavaScript using Jint Engine",
    author: "DesktopKomik",
    iconBackground: "#0d0f12",
    ...
    getSignatureHeaders: function(path) {
        ...
        var key = timestamp + "GET" + path + "WM_WEB_FRONT_END" + "xxxoidj";
        var message = "wm-api-request";
        var signature = sha256.hmac(key, message);
        return {
            "x-wm-request-time": timestamp,
            "x-wm-accses-key": "WM_WEB_FRONT_END",
    ...
    getPopularManga: function(page) {
        return this.getApiMangaPage(page, "&orderBy=Popular");
    },

    getLatestUpdates: function(page) {
        return this.getApiMangaPage(page, "&orderBy=Update");
    },
    ...
    getRawApiMangaPage: function(page, params) {
        ...
        var lastPage = paginator.last_page || page;
    ...
    getMangaDetails: function(url) {
        var parts = url.split("/");
        var slug = parts[2] || parts[1];
        
        var path = "/api/comic/" + slug;
        var fetchUrl = this.apiUrl + path;
        var headers = this.getSignatureHeaders(path);
        ...
        if (data.status === "ongoing") {
            ...
            status = 2;
        } else if (data.status === "hiatus") {
            status = 6;
        }
        
        var genres = [];
        if (data.country === "JP") genres.push("Manga");
        else if (data.country === "CN") genres.push("Manhua");
        else if (data.country === "KR") genres.push("Manhwa");
        
        if (data.color === true) genres.push("Colored");
        ...
        var description = "";
        if (data.synopsis) {
            description += data.synopsis.replace(/<[^>]+>/g, "").trim();
        }
        if (data.alternative_name) {
            if (description !== "") description += "\n\n";
            description += "Alternative Name: " + data.alternative_name.trim();
        }
    ...
    getChapterList: function(url) {
        ...
        for (var i = 0; i < data.chapters.length; i++) {
            ...
            var dateUpload = 0;
            if (ch.updated_at && ch.updated_at.time) {
                dateUpload = ch.updated_at.time * 1000;
            }
            chapters.push({
