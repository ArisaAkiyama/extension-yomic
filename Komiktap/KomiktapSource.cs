...
namespace Yomic.Extensions.Komiktap
{
    public class KomiktapSource : HttpSource, IFilterableMangaSource
    {
        public override string Name => "Komiktap";
        public override string BaseUrl => "https://komiktap.info";
        public override string Language => "ID";
        public override bool IsHasMorePages => true;

        public override string Version => "1.0.0";
        public override string IconUrl => "https://www.google.com/s2/favicons?domain=komiktap.info&sz=128";
        public override string Description => "Baca Manga Bahasa Indonesia di Komiktap (MangaThemesia)";
        public override string Author => "Yomic Desktop";
        public override string IconBackground => "#0c0d12";
        ...
        private async Task<HtmlDocument> GetHtmlAsync(string url)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Console.WriteLine($"[Komiktap] Bypass Failed: {bypassEx.Message}");
                    throw;
                }
            }
        ...
        public override async Task<List<Chapter>> GetChapterListAsync(string mangaId)
        {
            ...
            {
                Console.WriteLine($"[Komiktap] GetChapterListAsync error: {ex.Message}");
            }
        ...
        public override async Task<List<string>> GetPageListAsync(string chapterUrl)
        {
            ...
            try
            {
                ...
                if (pages.Count == 0)
                {
                    ...
                    if (jsonMatch.Success)
                    {
                        ...
                        {
                            Console.WriteLine($"[Komiktap] ts_reader JSON parse error: {jsonEx.Message}");
                        }
                    }
                }

                // Strategy 3: script with base64 encoded src (sometimes used in MangaThemesia)
                if (pages.Count == 0)
                {
                    ...
                    if (scriptNode != null)
                    {
                        ...
                        {
                            Console.WriteLine($"[Komiktap] Failed to parse base64 ts_reader script: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine($"[Komiktap] GetPageListAsync error: {ex.Message}");
            }
        ...
        private async Task<(List<Manga> Items, int TotalPages)> ScrapeMangaList(string url)
        {
            ...
            {
                Console.WriteLine($"[Komiktap] ScrapeMangaList error: {ex.Message}");
            }
