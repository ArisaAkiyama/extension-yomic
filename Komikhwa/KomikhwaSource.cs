...
namespace Yomic.Extensions.Komikhwa
{
    public class KomikhwaSource : HttpSource, IFilterableMangaSource
    {
        public override string Name => "Komikhwa";
        public override string BaseUrl => "https://komikhwa.com";
        public override string Language => "ID";
        public override bool IsHasMorePages => true;

        public override string Version => "1.0.0";
        public override string IconUrl => "https://www.google.com/s2/favicons?domain=komikhwa.com&sz=128";
        public override string Description => "Baca Manga Bahasa Indonesia di Komikhwa (MangaThemesia)";
        public override string Author => "Yomic Desktop";
        public override string IconBackground => "#ffffff";
        public override string IconForeground => "#000000";
        ...
        {
            Console.WriteLine($"[Komikhwa] GET {url}");
            string html = "";
            try 
            {
            ...
            {
                Console.WriteLine($"[Komikhwa] Access Denied ({ex.StatusCode}). Attempting Cloudflare Bypass...");
                try 
                {
                ...
                {
                    Console.WriteLine($"[Komikhwa] Bypass Failed: {bypassEx.Message}");
                    throw;
                }
            }
        ...
        public override async Task<List<Chapter>> GetChapterListAsync(string mangaId)
        {
            ...
            {
                Console.WriteLine($"[Komikhwa] GetChapterListAsync error: {ex.Message}");
            }
        ...
        public override async Task<List<string>> GetPageListAsync(string chapterUrl)
        {
            ...
            try
            {
                ...
                if (pages.Count == 0)
                {
                    ...
                    if (jsonMatch.Success)
                    {
                        ...
                        {
                            Console.WriteLine($"[Komikhwa] ts_reader JSON parse error: {jsonEx.Message}");
                        }
                    }
                }

                // Strategy 3: script with base64 encoded src (Komikhwa specific logic)
                if (pages.Count == 0)
                {
                    var scriptNode = doc.DocumentNode.SelectSingleNode("//script[starts-with(@src, 'data:text/javascript;base64,dHNfcmVhZGVyLnJ1bih7')]");
                    if (scriptNode != null)
                    {
                        string src = scriptNode.GetAttributeValue("src", "");
                        try
                        {
                            int base64Idx = src.IndexOf("base64,");
                            if (base64Idx >= 0)
                            {
                                string base64Str = src.Substring(base64Idx + 7);
                                byte[] bytes = Convert.FromBase64String(base64Str);
                                string decodedJs = System.Text.Encoding.UTF8.GetString(bytes);

                                var jsonMatch = Regex.Match(decodedJs, @"ts_reader\.run\((\{.*?\})\);", RegexOptions.Singleline);
                                if (jsonMatch.Success)
                                {
                                    string jsonStr = jsonMatch.Groups[1].Value;
                                    using var jsonDoc = JsonDocument.Parse(jsonStr);
                        ...
                        {
                            Console.WriteLine($"[Komikhwa] Failed to parse base64 ts_reader script: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine($"[Komikhwa] GetPageListAsync error: {ex.Message}");
            }
        ...
        private async Task<(List<Manga> Items, int TotalPages)> ScrapeMangaList(string url)
        {
            ...
            {
                Console.WriteLine($"[Komikhwa] ScrapeMangaList error: {ex.Message}");
            }
