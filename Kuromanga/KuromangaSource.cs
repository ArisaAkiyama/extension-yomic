...
namespace Yomic.Extensions.Kuromanga
{
    public class KuromangaSource : HttpSource, IFilterableMangaSource
    {
        public override string Name => "Kuromanga";
        public override string BaseUrl => "https://kuromanga.me";
        public override string Language => "ID";
        public override bool IsHasMorePages => true;

        public override string Version => "1.2.1";
        public override string IconUrl => "https://www.google.com/s2/favicons?domain=kuromanga.me&sz=128";
        public override string Description => "Baca Manga Bahasa Indonesia di Kuromanga (MangaThemesia)";
        ...
        {
            Console.WriteLine($"[Kuromanga] GET {url}");
            string html = "";
            try 
            {
            ...
            {
                Console.WriteLine($"[Kuromanga] Access Denied ({ex.StatusCode}). Attempting Cloudflare Bypass...");
                try 
                {
                ...
                {
                    Console.WriteLine($"[Kuromanga] Bypass Failed: {bypassEx.Message}");
                    throw;
                }
            }
        ...
        public override async Task<List<Chapter>> GetChapterListAsync(string mangaId)
        {
            ...
            {
                Console.WriteLine($"[Kuromanga] GetChapterListAsync error: {ex.Message}");
            }
        ...
        public override async Task<List<string>> GetPageListAsync(string chapterUrl)
        {
            ...
            try
            {
                ...
                if (pages.Count == 0)
                {
                    ...
                    if (jsonMatch.Success)
                    {
                        ...
                        {
                            Console.WriteLine($"[Kuromanga] ts_reader JSON parse error: {jsonEx.Message}");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine($"[Kuromanga] GetPageListAsync error: {ex.Message}");
            }
        ...
        private async Task<(List<Manga> Items, int TotalPages)> ScrapeMangaList(string url)
        {
            ...
            {
                Console.WriteLine($"[Kuromanga] ScrapeMangaList error: {ex.Message}");
            }
