var source = {
    ...
    getSeriesPage: function(page, query, status, genre, type, order) {
        ...
        let cards = doc.querySelectorAll("div.bsx, div.utao, div.animposx, div.manga-item, .page-item-detail");
        let items = [];
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            if (!title) {
                let ttEl = card.querySelector(".tt, .title, h3, h2");
            ...
            if (statusEl) {
                ...
                else if (sText.includes("completed")) statusVal = 2;
            }
    ...
    getMangaDetails: function(url) {
        ...
        let title = "";
        let titleEl = doc.querySelector("h1.entry-title, h1.manga-title, h1");
        if (titleEl) title = titleEl.text().trim();

        let thumbnailUrl = "";
        let thumbEl = doc.querySelector(".thumb img, .cover img, img.wp-post-image");
        if (thumbEl) {
        ...
        let descEl = doc.querySelector(".entry-content, div[itemprop='description'], .desc, .synopsis, .summary__content");
        let description = descEl ? descEl.text().trim() : "";

        let author = "";
        let infoEls = doc.querySelectorAll(".tsinfo .imethod, .infotable tr, .spe span, .imptdt");
        ...
        if (bodyLower.includes("status: ongoing") || bodyLower.includes(">ongoing<")) status = 1;
        else if (bodyLower.includes("status: completed") || bodyLower.includes(">completed<")) status = 2;
        else if (bodyLower.includes("status: hiatus")) status = 3;

        let genres = [];
        let genreEls = doc.querySelectorAll(".mgen a, .genres-container a, a[rel='tag'], .genre-info a");
    ...
    getChapterList: function(mangaUrl) {
        ...
        let doc = Html.parse(response.body, fullUrl);
        let items = doc.querySelectorAll("#chapterlist ul li, .eplister ul li, ul.cl-ul li, .chapter-list li");
        let chapters = [];
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            let nameEl = item.querySelector(".chapternum, .chapter-title");
            let name = nameEl ? nameEl.text().trim() : linkEl.text().trim();

            let dateUpload = 0;
            let dateEl = item.querySelector(".chapterdate, .chapter-date");
            if (dateEl) {
                let parsedDate = Date.parse(dateEl.text().trim());
                if (!isNaN(parsedDate)) dateUpload = parsedDate;
            }

            chapters.push({ name: name, url: relativeUrl, dateUpload: dateUpload });
        }
