...
namespace Yomic.Extensions.Weebcentral
{
    public class WeebcentralSource : HttpSource, IFilterableMangaSource
    {
        ...
        public override string IconUrl => "https://www.google.com/s2/favicons?domain=weebcentral.com&sz=128";
        ...
        public override async Task<List<Manga>> GetPopularMangaAsync(int page)
        {
            ...
            var url = $"{BaseUrl}/search/data?sort=Popularity&order=Descending&official=Any&anime=Any&adult=Any&display_mode=Full+Display&offset={offset}&limit=32";
        ...
        {
            return (await GetPopularMangaAsync(page), 280);
        }

        public override async Task<List<Manga>> GetSearchMangaAsync(string query, int page)
        {
            int offset = (page - 1) * 32;
            var url = $"{BaseUrl}/search/data?text={Uri.EscapeDataString(query)}&sort=Popularity&order=Descending&official=Any&anime=Any&adult=Any&display_mode=Full+Display&offset={offset}&limit=32";
            var doc = await GetHtmlAsync(url);
            return ParseMangaList(doc);
        }
        ...
        public override async Task<Manga> GetMangaDetailsAsync(string id)
        {
            ...
            var manga = new Manga
            {
                Url = id,
                Source = this.Id,
                Title = System.Net.WebUtility.HtmlDecode(root.SelectSingleNode("//h1")?.InnerText?.Trim() ?? "Unknown")
            };
            ...
            manga.Description = System.Net.WebUtility.HtmlDecode(synopsisNode?.InnerText?.Trim() ?? "");
        ...
        public override async Task<List<Chapter>> GetChapterListAsync(string id)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                // 1. Decode any HTML entities like &#39; to '
                title = System.Net.WebUtility.HtmlDecode(title);

                // 2. Weebcentral's InnerText might contain "Last Read" and dates on new lines. Take only the first line.
                var firstLine = title.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault();
                title = firstLine?.Trim() ?? title;

                // 3. Convert Weebcentral's "Break: 92" or "Break 92" titles to "Chapter 92"
                title = System.Text.RegularExpressions.Regex.Replace(title, @"(?i)Break:?", "Chapter");
